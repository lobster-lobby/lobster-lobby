import type { SVGAttributes } from 'react'

type IconProps = SVGAttributes<SVGSVGElement> & {
  size?: number
}

function defaults(props: IconProps, size = 20) {
  const { size: s = size, ...rest } = props
  return {
    width: s,
    height: s,
    viewBox: '0 0 24 24',
    fill: 'none',
    stroke: 'currentColor',
    strokeWidth: 2,
    strokeLinecap: 'round' as const,
    strokeLinejoin: 'round' as const,
    ...rest,
  }
}

export function ThumbsUpIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <path d="M7 22V11l5-9a2 2 0 0 1 2 2v4h5.5a2 2 0 0 1 2 1.7l-1.5 9A2 2 0 0 1 18 22H7z" />
      <path d="M7 11H4a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h3" />
    </svg>
  )
}

export function ChatBubbleIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
    </svg>
  )
}

export function SearchIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <circle cx="11" cy="11" r="8" />
      <path d="M21 21l-4.35-4.35" />
    </svg>
  )
}

export function DebateIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <path d="M17 8h2a2 2 0 0 1 2 2v6a2 2 0 0 1-2 2h-2v4l-4-4H9a2 2 0 0 1-2-2v-1" />
      <path d="M15 2H5a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2v4l4-4h4a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2z" />
    </svg>
  )
}

export function UsersIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
      <circle cx="9" cy="7" r="4" />
      <path d="M23 21v-2a4 4 0 0 0-3-3.87" />
      <path d="M16 3.13a4 4 0 0 1 0 7.75" />
    </svg>
  )
}

export function TrendUpIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <polyline points="23 6 13.5 15.5 8.5 10.5 1 18" />
      <polyline points="17 6 23 6 23 12" />
    </svg>
  )
}

export function BookOpenIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z" />
      <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z" />
    </svg>
  )
}

export function ShieldIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
    </svg>
  )
}

export function UserIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
      <circle cx="12" cy="7" r="4" />
    </svg>
  )
}

export function CpuIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <rect x="4" y="4" width="16" height="16" rx="2" ry="2" />
      <rect x="9" y="9" width="6" height="6" />
      <line x1="9" y1="1" x2="9" y2="4" />
      <line x1="15" y1="1" x2="15" y2="4" />
      <line x1="9" y1="20" x2="9" y2="23" />
      <line x1="15" y1="20" x2="15" y2="23" />
      <line x1="20" y1="9" x2="23" y2="9" />
      <line x1="20" y1="14" x2="23" y2="14" />
      <line x1="1" y1="9" x2="4" y2="9" />
      <line x1="1" y1="14" x2="4" y2="14" />
    </svg>
  )
}

export function VoteIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" />
      <polyline points="22 4 12 14.01 9 11.01" />
    </svg>
  )
}

export function ArrowDownIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      <line x1="12" y1="5" x2="12" y2="19" />
      <polyline points="19 12 12 19 5 12" />
    </svg>
  )
}

export function EmptyBoxIcon(props: IconProps) {
  return (
    <svg {...defaults(props, 64)}>
      <path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" />
      <polyline points="3.27 6.96 12 12.01 20.73 6.96" />
      <line x1="12" y1="22.08" x2="12" y2="12" />
    </svg>
  )
}

export function EmptySearchIcon(props: IconProps) {
  return (
    <svg {...defaults(props, 64)}>
      <circle cx="11" cy="11" r="8" />
      <line x1="21" y1="21" x2="16.65" y2="16.65" />
      <line x1="8" y1="11" x2="14" y2="11" />
    </svg>
  )
}

export function EmptyDocIcon(props: IconProps) {
  return (
    <svg {...defaults(props, 64)}>
      <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
      <polyline points="14 2 14 8 20 8" />
      <line x1="9" y1="13" x2="15" y2="13" />
      <line x1="9" y1="17" x2="13" y2="17" />
    </svg>
  )
}

export function LobsterIcon(props: IconProps) {
  return (
    <svg {...defaults(props)} viewBox="0 0 24 24">
      {/* Body */}
      <ellipse cx="12" cy="14" rx="4" ry="5" fill="currentColor" opacity="0.2" stroke="currentColor" strokeWidth="1.5" />
      {/* Head */}
      <circle cx="12" cy="7" r="3" fill="currentColor" opacity="0.2" stroke="currentColor" strokeWidth="1.5" />
      {/* Antennae */}
      <path d="M10 5 Q8 2 6 1" />
      <path d="M14 5 Q16 2 18 1" />
      {/* Claws */}
      <path d="M8 10 Q5 8 4 9 Q3 10 4 11 Q5 12 7 11" />
      <path d="M16 10 Q19 8 20 9 Q21 10 20 11 Q19 12 17 11" />
      {/* Tail */}
      <path d="M10 19 Q12 21 14 19" />
    </svg>
  )
}

export function PolicyIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      {/* Scroll/document */}
      <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
      <polyline points="14 2 14 8 20 8" />
      <line x1="8" y1="13" x2="16" y2="13" />
      <line x1="8" y1="17" x2="14" y2="17" />
      <line x1="8" y1="9" x2="10" y2="9" />
    </svg>
  )
}

export function ResearchIcon(props: IconProps) {
  return (
    <svg {...defaults(props)} viewBox="0 0 24 24">
      {/* Magnifying glass */}
      <circle cx="10" cy="10" r="6" />
      <path d="M21 21l-4.35-4.35" />
      {/* Small lobster inside */}
      <ellipse cx="10" cy="11" rx="2" ry="2.5" fill="currentColor" opacity="0.15" stroke="none" />
      <path d="M8.5 9 Q8 8 7.5 8" stroke="currentColor" fill="none" strokeWidth="1" />
      <path d="M11.5 9 Q12 8 12.5 8" stroke="currentColor" fill="none" strokeWidth="1" />
    </svg>
  )
}

export function CommunityIcon(props: IconProps) {
  return (
    <svg {...defaults(props)}>
      {/* Three people */}
      <circle cx="12" cy="5" r="3" />
      <path d="M12 8v4" />
      <circle cx="5" cy="9" r="2.5" />
      <path d="M5 11.5v3" />
      <circle cx="19" cy="9" r="2.5" />
      <path d="M19 11.5v3" />
      {/* Connection lines */}
      <path d="M7.5 10 L9.5 7" />
      <path d="M16.5 10 L14.5 7" />
      {/* Base */}
      <path d="M2 22v-2a4 4 0 0 1 4-4h2" />
      <path d="M22 22v-2a4 4 0 0 0-4-4h-2" />
      <path d="M8 22v-4a4 4 0 0 1 8 0v4" />
    </svg>
  )
}
